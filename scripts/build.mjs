...
async function buildFile(filename, useBlank) {
    const content = await buildContent(useBlank);
    await writeFile(`dist/${filename}`, content);
    console.log(`dist/${filename} has been generated`);
    ...
    await writeFile(`dist/minified/${filename}`, minifiedContent);
    console.log(`dist/minified/${filename} has been generated`);
}
...
async function main() {
    ...
    await buildFile('blank.html', true);
    await writeFile(noJekyllFile, "");
    console.log(`${noJekyllFile} has been created`);
    await writeFile(
        octocatFile,
    ...
    console.log(`${octocatFile} has been created`);
}
